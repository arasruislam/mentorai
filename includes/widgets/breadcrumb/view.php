...
$separator_type  = isset( $settings['separator_type'] ) ? (string) $settings['separator_type'] : 'text';
...
/**
 * Build breadcrumb items: [ ['label' => string, 'url' => string], ... ]
 * url empty => non-link (current)
 */
if ( is_front_page() ) {
	if ( $show_home ) {
	...
	if ( is_home() ) {
		...
		if ( $post ) {
			...
			// Posts: category chain (simple first category)
			if ( 'post' === $post_type ) {
...
if (
	'icon' === $separator_type &&
	is_array( $separator_icon ) &&
	isset( $separator_icon['value'] )
) {
	if (
		( is_string( $separator_icon['value'] ) && '' !== trim( $separator_icon['value'] ) )
		|| is_array( $separator_icon['value'] )
	) {
		$has_icon = true;
	}
}
